...
#把包含视频的内容返回，参数为网页源代码
def get_vedio_content(html,tpye_num):
    # . 匹配任意字符
    # * 匹配匹配前面的元字符的0到n次
    # ? 匹配前面的子表达式0次或1次
    # .* 贪婪模式 
    # .*? 非贪婪模式（最小匹配）
    # re.S 正则表达式可以匹配到换行
    reg = re.compile(r'(<li><a href="/move/' + str(tpye_num) + '/.*?</h3>)',re.S)
...
def get_end_path(response,type_num):
    reg = r'<li><a href="/move/' + str(type_num) + '/(.*?)"'
    return re.findall(reg,response)
...
def download_vedio(vedio_url,vedio_name,vedio_type_name):
    ...
    print(vedio_name)
    save_path = 'F:/internet spider/game5/game9/game7/game0/game0/game9/game6/' + vedio_type_name + '/{}.mp4'.format(vedio_name)
...
# 运行一个网页
def run_curr_web(vedio_content,vedio_type_name,type_num):
    # 当前页计数器
    ...
    for one_vedio_content in vedio_content:
        try:
            ...
            end_path = get_end_path(one_vedio_content,type_num)[0]
            if end_path != '':
                #得到单个视频网页的地址
                one_path = 'https://www.9366df.com/move/' + str(type_num) + '/' + end_path
...
def main(start_urls,vedio_type_name,type_num):
    # 页数 计数器
    count_page = 0
    for start_url in start_urls: #进入每一页
        try:
            ...
            vedio_content = get_vedio_content(html_text,vedio_type)
            # 进入run_curr_web() 处理每页的18个视频内容
            run_curr_web(vedio_content,vedio_type_name,type_num)
        except:
            continue
...
if __name__ == '__main__':
    ...
    max_page = 5
    '''
    vedio_type = 1 对应 max_page 的最大值为 2620 
    vedio_type = 3 对应 max_page 的最大值为 2860   
    vedio_type = 7 对应 max_page 的最大值为 3930
    '''
    
    #列表生成器，生成 1到max_page个网页url
    start_urls = ['https://www.9366df.com/move/' + str(vedio_type) +
                  '/index_{}.html'.format(i) for i in range(start_page,max_page)]
    #第一页比较特殊，最后插入
    start_urls.insert(0,'https://www.9366df.com/move/' + str(vedio_type) +  '/index.html')
    
    path = 'F:/internet spider/game5/game9/game7/game0/game0/game9/game6/' + type_dict[vedio_type]
