...
if __name__ == '__main__':
    ...
    for i in range(len(file_name)):
        try:
            ...
            outResult('SEMIFS_MM',file_name[i], selected_num[i], step[i], 'o', 'BDNB')
            outResult('CHI', file_name[i], selected_num[i], step[i], 'o', 'BDNB')
            outResult('f_score', file_name[i], selected_num[i], step[i], 'o', 'BDNB')
            outResult('fisher', file_name[i], selected_num[i], step[i], 'o', 'BDNB')
            outResult('IG', file_name[i], selected_num[i], step[i], 'o', 'BDNB')
            outResult('MCFS', file_name[i], selected_num[i], step[i], 'o', 'BDNB')
            outResult('reliefF', file_name[i], selected_num[i], step[i], 'o', 'BDNB')
            outResult('SPEC', file_name[i], selected_num[i], step[i], 'o', 'BDNB')
        except:
    ...
    for i in range(len(file_name)):
        try:
            print('---------**** runing : ', '第 ', i + 1, ' 个数据集 *******---------')
            outResult('SEMIFS_MM',file_name[i], selected_num[i], step[i], 'm', 'BDNB')
            outResult('CHI', file_name[i], selected_num[i], step[i], 'm', 'BDNB')
            outResult('f_score', file_name[i], selected_num[i], step[i], 'm', 'BDNB')
            outResult('fisher', file_name[i], selected_num[i], step[i], 'm', 'BDNB')
            outResult('IG', file_name[i], selected_num[i], step[i], 'm', 'BDNB')
            outResult('MCFS', file_name[i], selected_num[i], step[i], 'm', 'BDNB')
            outResult('reliefF', file_name[i], selected_num[i], step[i], 'm', 'BDNB')
            outResult('SPEC', file_name[i], selected_num[i], step[i], 'm', 'BDNB')
        except:
            print('this dataset arise error')
            continue
