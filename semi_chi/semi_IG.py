...
def SEMIIG(X_train, y_train):
    F1 = IG.IGfs(X_train, y_train)
...
def getACC(size,file_name,X, y, selected_num,classifier = 'Gauss',k = 10):
    ...
    for train_index,test_index in skf.split(X,y):
        ...
        feature_score = SEMIIG(X_train, y_train)
        wd.out_feature_score('SEMIFS_IG',feature_score, classifier, file_name, count,size)
        ...
        acc += score
    acc = acc / k
    print(selected_num,'\t', 'SEMIFS_IG', '\t', acc)
    return acc
...
if __name__ == '__main__':
    ...
    for i in range(len(file_name)):
        try:
            ...
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 's', 'Gauss')
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 's', 'GBC')
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 's', 'Tree')
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 's', 'KNN')
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 's', 'RFC')
        except:
    ...
    for i in range(len(file_name)):
        try:
            print('---------**** runing : ', '第 ', i + 1, ' 个数据集 *******---------')
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 'm', 'Gauss')
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 'm', 'GBC')
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 'm', 'Tree')
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 'm', 'KNN')
            test_Gauss.outResult('SEMIFS_IG', file_name[i], selected_num[i], step[i], 'm', 'RFC')
        except:
            print('this dataset arise error')
            continue
