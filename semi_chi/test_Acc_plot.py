...
# Gauss   GBC  KNN  RFC
    ...
    for i in range(len(file_name)):
        ...
        for j in range(len(method)):
            acc_one_1 = getAccList(method[j], file_name[i], 'm', 'Gauss')  # 换大小数据集需要修改
            acc_one_1 = np.array(acc_one_1)
            acc_one_2 = getAccList(method[j], file_name[i], 'm', 'GBC')
            acc_one_2 = np.array(acc_one_2)
            acc_one_3 = getAccList(method[j], file_name[i], 'm', 'KNN')
            acc_one_3 = np.array(acc_one_3)
            acc_one_4 = getAccList(method[j], file_name[i], 'm', 'RFC')
