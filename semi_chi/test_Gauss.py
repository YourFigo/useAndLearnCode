...
def outResult(FS,file_name,selected_num,step = 10,size = 's',classifier = 'Gauss',k = 10):
    ...
    for i in range(1, selected_num + 1, step):
        ...
        if FS == 'SEMIFS_IG':
            acc.append(semi_IG.getACC(size,file_name,X, y, i,classifier,k))
        if FS == 'SEMIFS_DV':
            acc.append(semi_chi.getACC(size,file_name,X, y, i,classifier,k))
        elif FS == 'CHI':
...
if __name__ == '__main__':
    ...
    for i in range(len(file_name)):
        try:
            ...
            outResult('SEMIFS_MM',file_name[i], selected_num[i], step[i], 's', 'Gauss')
            outResult('CHI', file_name[i], selected_num[i], step[i], 's', 'Gauss')
            outResult('f_score', file_name[i], selected_num[i], step[i], 's', 'Gauss')
            outResult('fisher', file_name[i], selected_num[i], step[i], 's', 'Gauss')
            outResult('IG', file_name[i], selected_num[i], step[i], 's', 'Gauss')
            outResult('MCFS', file_name[i], selected_num[i], step[i], 's', 'Gauss')
            outResult('reliefF', file_name[i], selected_num[i], step[i], 's', 'Gauss')
            outResult('SPEC', file_name[i], selected_num[i], step[i], 's', 'Gauss')
        except:
    ...
    #     try:
    #         print('---------**** runing : ', '第 ', i + 1, ' 个数据集 *******---------')
    #         outResult('SEMIFS_MM',file_name[i], selected_num[i], step[i], 'o', 'Gauss')
    #         outResult('CHI', file_name[i], selected_num[i], step[i], 'o', 'Gauss')
    #         outResult('f_score', file_name[i], selected_num[i], step[i], 'o', 'Gauss')
    #         outResult('fisher', file_name[i], selected_num[i], step[i], 'o', 'Gauss')
    #         outResult('IG', file_name[i], selected_num[i], step[i], 'o', 'Gauss')
    #         outResult('MCFS', file_name[i], selected_num[i], step[i], 'o', 'Gauss')
    #         outResult('reliefF', file_name[i], selected_num[i], step[i], 'o', 'Gauss')
    #         outResult('SPEC', file_name[i], selected_num[i], step[i], 'o', 'Gauss')
    #     except:
    #         print('this dataset arise error')
    #         continue
    #
    ...
    #     try:
    #         print('---------**** runing : ', '第 ', i + 1, ' 个数据集 *******---------')
    #         outResult('SEMIFS_MM',file_name[i], selected_num[i], step[i], 'm', 'Gauss')
    #         outResult('CHI', file_name[i], selected_num[i], step[i], 'm', 'Gauss')
    #         outResult('f_score', file_name[i], selected_num[i], step[i], 'm', 'Gauss')
    #         outResult('fisher', file_name[i], selected_num[i], step[i], 'm', 'Gauss')
    #         outResult('IG', file_name[i], selected_num[i], step[i], 'm', 'Gauss')
    #         outResult('MCFS', file_name[i], selected_num[i], step[i], 'm', 'Gauss')
    #         outResult('reliefF', file_name[i], selected_num[i], step[i], 'm', 'Gauss')
    #         outResult('SPEC', file_name[i], selected_num[i], step[i], 'm', 'Gauss')
    #     except:
    #         print('this dataset arise error')
    #         continue
